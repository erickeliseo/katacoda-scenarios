...
Create a Cassandra [StatefulSet](https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/) that uses a Rook PVC.

Take a look at the yaml:
```cat ~/files/cassandra-statefulset.yaml```{{execute T1}}
...
```
kubectl create -f ~/files/cassandra-statefulset.yaml
```{{execute T1}}
